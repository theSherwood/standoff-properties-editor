(function (factory) {
  ...
  function whileNext(startNode, endNode, func) {
    ...
    startNode = startNode.speedy
      ? startNode
      : getParent(startNode, (n) => n.speedy);
  ...
  function firstPreviousChar(start) {
    var char = previousUntil(start, (node) => {
      return (
        node.speedy.role == ELEMENT_ROLE.CHAR &&
        node.speedy.stream == TEXT_STREAM.IN &&
        !node.speedy.isLineBreak
      );
    });
    return char;
  }
  ...
  function unsetSpanRange(span, className) {
    ...
    var div = first(nodelist, function (x) {
  ...
  function hideLayer(span, layer) {
    var nodelist = span.children;
    var div = first(nodelist, function (x) {
      return x.getAttribute("data-layer") == layer;
    });
    if (div) {
  ...
  var Property = (function () {
    ...
    Property.prototype.unsetSpanRange = function () {
      ...
      if (propertyType.format == "block") {
        ...
        var insertionPoint = parent.nextElementSibling
        ...
        whileNext(
          ...
          function (s) {
            var className =
              ...
              propertyType.zeroPoint.className;
            if (propertyType.format == "decorate") {
              s.classList.remove(className);
            } else if (propertyType.format == "overlay") {
      ...
      if (propertyType.unstyleRenderer) {
        propertyType.unstyleRenderer(
          allNodesBetween(this.startNode, this.endNode),
          this
        );
      }
    };
      ...
      whileNext(
        ...
        function (s) {
          if (!_this.isZeroPoint) {
            if (
              s.speedy.role != ELEMENT_ROLE.CHAR ||
              s.speedy.stream != TEXT_STREAM.IN
            ) {
              return;
            }
          }
  ...
  var Editor = (function () {
    function Editor(cons) {
      this.container =
        cons.container instanceof HTMLElement
          ? cons.container
          : document.getElementById(cons.container);
    ...
    Editor.prototype.shiftPropertiesLeftFromNode = function (node) {
      ...
      var properties = this.data.properties.filter((x) => x.startIndex() > i);
    ...
    Editor.prototype.setupEventHandlers = function () {
      ...
      this.container.addEventListener(
        "paste",
        this.handleOnPasteEvent.bind(this)
      );
    };
    Editor.prototype.setAnimationFrame = function () {
      var _this = this;
      window.requestAnimationFrame(function () {
        var properties = _this.data.properties.filter(
          (p) => !!_this.propertyType[p.type].onRequestAnimationFrame
        );
        ...
        properties.forEach((p) =>
          _this.propertyType[p.type].onRequestAnimationFrame(
            p,
            _this.propertyType[p.type],
            _this
          )
        );
      });
    ...
    Editor.prototype.handleDoubleClickEvent = function (e) {
      ...
      var props = this.data.properties.filter(function (prop) {
        ...
        return (
          propertyType &&
          propertyType.propertyValueSelector &&
          isWithin(prop.startNode, prop.endNode, e.target)
        );
      });
      if (!props.length) {
        return;
      ...
      window.setTimeout(
        function () {
          ...
          if (!_this.marked) {
            ...
            _this.data.properties.sort((a, b) =>
    ...
    Editor.prototype.deleteAnnotation = function (type) {
      ...
      var enclosing = this.data.properties.filter(function (prop) {
        return (
          !prop.isDeleted &&
          prop.type == type &&
          isWithin(prop.startNode, prop.endNode, current)
        );
      });
    ...
    Editor.prototype.setMonitor = function (props) {
      ...
      window.setTimeout(function () {
        _this.monitors.forEach((x) =>
          x.update({
            properties: props,
            characterCount: _this.characterCount,
            editor: _this,
          })
        );
      }, 1);
    ...
    Editor.prototype.updateSelectors = function (evt) {
      ...
      if (selection) {
        ...
        var properties = this.getPropertiesWithin(
          selection.start,
          selection.end
        );
        ...
        if (this.selectors) {
          this.selectors.forEach((s) =>
            s({ editor: this, properties, selection })
          );
        }
      } else {
    ...
    Editor.prototype.insertCharacterAtCarot = function (c) {
      ...
      if (isFirst) {
        ...
        var next = atFirst
          ...
          : this.getNextCharacterNode(current);
    ...
    Editor.prototype.getPropertiesTraversingRange = function (
      startNode,
      endNode
    ) {
      var traversing = [];
      ...
      if (previous) {
        if (previous.endProperties.length) {
          previous.endProperties
            .filter(function (ep) {
              return ep.startNode == next && ep.endNode == previous;
            })
            .forEach(function (single) {
              remove(_.data.properties, single);
            });
        }
      }
      current.remove();
    ...
    Editor.prototype.handleDelete = function (current) {
      ...
      next.startProperties
        .filter(function (sp) {
          return sp.endNode == current && sp.startNode == next;
        })
        .forEach(function (single) {
          remove(_.data.properties, single);
        });
    ...
    Editor.prototype.leftSelection = function (evt, current) {
      ...
      if (this.mode.selection.direction == SELECTION_DIRECTION.LEFT) {
        ...
        if (!this.mode.selection.start) {
          ...
          var previous = this.getPreviousCharacterNode(
            this.mode.selection.start
          );
          this.mode.selection.start = previous;
        }
    ...
    Editor.prototype.handleKeyDownEvent = function (evt) {
      ...
      if (PASSTHROUGH_CHARS.indexOf(key) >= 0) {
        ...
        if (canAnnotate) {
          ...
          var propertyTypeName = this.getPropertyTypeNameFromShortcutKey(
            evt.key
          );
          if (propertyTypeName) {
            evt.preventDefault();
      ...
      if (isFirst) {
        ...
        if (!atFirst) {
          var index = current
            ? nodeIndex(current)
            : nodeIndex(this.getPreviousCharacterNode(current));
    ...
    Editor.prototype.getSelectionNodes = function () {
      ...
      var startNode = getParent(
        startContainer,
        (x) => x.speedy && x.speedy.role == ELEMENT_ROLE.CHAR
      );
      var endNode = getParent(
        endContainer,
        (x) => x.speedy && x.speedy.role == ELEMENT_ROLE.CHAR
      );
      ...
      if (propertyType) {
        if (propertyType.bracket) {
          if (propertyType.bracket.left) {
            var left = this.createBracketNode(
              prop,
              propertyType.bracket.left,
              sn.parentElement,
              sn
            );
          ...
          if (propertyType.bracket.right) {
            var right = this.createBracketNode(
              prop,
              propertyType.bracket.right,
              en.parentElement,
              en.nextElementSibling
            );
    ...
    Editor.prototype.createZeroPointProperty = function (
      propertyTypeName,
      content
    ) {
      var _this = this;
      var type = find(this.propertyType, function (__, key) {
        return key == propertyTypeName;
      });
      if (!type) {
    ...
    Editor.prototype.createProperty = function (
      propertyTypeName,
      value,
      propertyRange
    ) {
      ...
      if (type.bracket) {
        ...
        if (type.bracket.right) {
          var right = this.createBracketNode(
            prop,
            type.bracket.right,
            range.end.parentElement,
            range.end.nextElementSibling
          );
          prop.addRightBracket(right);
        }
      }
    ...
    Editor.prototype.paint = function (s, i) {
      ...
      var properties = this.data.properties
        .filter(function (prop) {
          return (
            !prop.isDeleted && prop.startIndex() <= i && i < prop.endIndex()
          );
        })
        .sort(function (a, b) {
    ...
    Editor.prototype.unbind = function () {
      var text = (this.temp.text = markNodesWithIndexes(
        this.container.firstChild
      ));
      var result = {
        text: text,
    ...
    Editor.prototype.createBracketNode = function (
      prop,
      bracket,
      parent,
      next
      ...
      var content =
        typeof bracket.content == "function"
          ? bracket.content(prop, this)
          : bracket.content;
    ...
    Editor.prototype.handleBlocks = function (properties) {
      ...
      var blocks = properties
        .filter(
          (item) =>
            !item.isDeleted &&
            _this.propertyType[item.type] &&
            _this.propertyType[item.type].format == "block"
        )
        .sort((a, b) =>
          a.startIndex > b.startIndex ? -1 : a.startIndex < b.startIndex ? 1 : 0
        );
      if (blocks.length) {
        // blocks.forEach(p => _this.createBlock(_this.container.children[p.startIndex], _this.container.children[p.endIndex], p.type));
        blocks.forEach((p) =>
          _this.createBlock(
            _this.indexNode(p.startIndex),
            _this.indexNode(p.endIndex),
            p.type
          )
        );
      }
    ...
    Editor.prototype.handleZeroLengthAnnotations = function (model) {
      ...
      zeroProperties = zeroProperties.sort((a, b) =>
        a.startIndex > b.startIndex ? -1 : a.startIndex < b.startIndex ? 1 : 0
      );
